import gen_ff_dataset as DL
from ff_config import config
...
class trainer:
    ...
    def renew_everything(self):
        # renew dataloader.
        self.loader = DL.dataFFLoader(config)
    ...
    def train(self):
        ...
        for step in range(config.start_res, self.max_resl+1+5):
            for iter in tqdm(range(0,(self.trns_tick*2+self.stab_tick*2)*self.TICK, self.loader.batchsize)):
                ...
                # save model.
                self.snapshot('repo_ff/model')

                # save image grid.
                if self.globalIter%self.config.save_img_every == 0:
                    x_test = self.G(self.z_test.float())
                    os.system('mkdir -p repo_ff/save/grid')
                    utils.save_image_grid(x_test.data, 'repo_ff/save/grid/{}_{}_G{}_D{}.jpg'.format(int(self.globalIter/self.config.save_img_every), self.phase, self.complete['gen'], self.complete['dis']))
                    os.system('mkdir -p repo_ff/save/resl_{}'.format(int(floor(self.resl))))
                    utils.save_image_single(x_test.data, 'repo_ff/save/resl_{}/{}_{}_G{}_D{}.jpg'.format(int(floor(self.resl)),int(self.globalIter/self.config.save_img_every), self.phase, self.complete['gen'], self.complete['dis']))
